...
static int send_to_daemon(const char *msg, int len, int type)
{
    ...
    if (len > 0 && msg)
        memcpy(nlmsg_data(nlh), msg, len); // copy payload to message segment
...
static bool ncd_queue_pending_locked(struct ncd_device *dev, const char *data, unsigned int len)
{
    ...
    if (len > FIFO_SIZE)
        return false;
    if (dev->pending_bytes > FIFO_PENDING_LIMIT - len)
        return false;
...
static unsigned int ncd_drain_pending_locked(struct ncd_device *dev)
{
    ...
    while (!list_empty(&dev->pending_chunks))
    {
        ...
        if (kfifo_avail(&dev->data_fifo) < chunk->len)
            break;
...
static void recv_from_daemon(struct sk_buff *skb)
{
    ...
    case NCD_MSG_DATA:
        ...
        if (nlh->nlmsg_len > NLMSG_HDRLEN)
        {
...

            {
                ...
                if (!list_empty(&dev->pending_chunks) || kfifo_avail(&dev->data_fifo) < payload_len)
                {
    ...
    case NCD_MSG_CREATE_DEV:
        ...
        }
        {
            ...
            /* create device */
            ncd_devices[minor]->device =
                device_create(ncd_class, NULL, ncd_devices[minor]->dev_num, NULL, ncd_devices[minor]->name);
...
static struct file_operations ncd_file_operations = {
    .owner = THIS_MODULE, .open = ncd_open, .read = ncd_read, .write = ncd_write, .release = ncd_release};

static char *ncd_devnode(const struct device *dev, umode_t *mode)
{
    if (mode)
